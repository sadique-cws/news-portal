...
const Insert = ({setBlog, blog}) => {
  ...
  const handleSubmit = () => {
      const newObj = {title:data.title, author: data.author, date:data.date, content:data.content}
      setBlog([...blog, newObj]);
      setData(InitObj)
  }
  return (
    <div className='w-1/4'>
        <div className='p-3'>
            ...
            <div className='mb-3 flex flex-col'>
                <button type='button' onClick={handleSubmit} className="bg-red-600 text-white p-3">Send</button>
            </div>
        </div>
    </div>
  )
